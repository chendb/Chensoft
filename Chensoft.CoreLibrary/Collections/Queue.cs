...
namespace Chensoft.Collections
{
	...
	/// </remarks>
	public class Queue : Chensoft.Collections.IQueue
	{
		#region 事件声明
		/// <summary>
		/// 表示入队成功的事件。
		...
		#region 构造函数
		/// <summary>
		/// 初始化<seealso cref="Chensoft.Collections.Queue"/>类的新实例，该实例为空，其初始容量为32和2.0f的成长因子。
		/// </summary>
		...
		/// <summary>
		/// 初始化<seealso cref="Chensoft.Collections.Queue"/>类的新实例，该实例为空，具有指定的初始容量和默认为2.0f的成长因子。
		/// </summary>
		...
		/// <summary>
		/// 初始化<seealso cref="Chensoft.Collections.Queue"/>类的新实例，该实例为空，具有指定的初始容量并使用指定的成长因子。
		/// </summary>
		/// <param name="capacity">初始化的队列容量，即包含的初始元素数。</param>
		...
		#region 出队操作
		/// <summary>
		/// 移除并返回位于<see cref="Chensoft.Collections.Queue"/>开始处的对象。
		/// </summary>
		...
		#region 入队操作
		/// <summary>
		/// 将指定集合中的所有元素依次添加到<seealso cref="Chensoft.Collections.Queue"/>的结尾处。
		/// </summary>
		...
		/// <summary>
		/// 将对象添加到<seealso cref="Chensoft.Collections.Queue"/>的结尾处。
		/// </summary>
		...
		///		<para>成长因子是当需要更大容量时当前容量要乘以的数字。在构造<seealso cref="Chensoft.Collections.Queue"/>时确定增长因子。无论增长因子是多少，队列的容量将始终增加一个最小值（即<see cref="MinimumGrow"/>属性值），即使1.0f的增长因子也不会阻止队列的扩容。</para>
		/// </remarks>
		...
		#region 激发事件
		protected virtual void OnDequeued(Chensoft.Collections.DequeuedEventArgs args)
		{
			if(this.Dequeued != null)
				this.Dequeued(this, args);
		}

		protected virtual void OnEnqueued(Chensoft.Collections.QueueEventArgs args)
		{
