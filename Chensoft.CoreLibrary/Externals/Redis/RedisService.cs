...
namespace Chensoft.Externals.Redis
{
	public class RedisService : MarshalByRefObject, IRedisService, IDisposable
	{
		...
		private Chensoft.Collections.ObjectPool<IRedisClient> _redisPool;
		private ConcurrentDictionary<int, ObjectCache<RedisObjectBase>> _caches;
		#endregion
		...
		private T GetCacheEntry<T>(string name, RedisEntryType entryType, Func<string, bool, T> createThunk) where T : RedisObjectBase
		{
			...
			//确保缓存容器创建完成
			if(_caches == null)
				System.Threading.Interlocked.CompareExchange(ref _caches, new ConcurrentDictionary<int, ObjectCache<RedisObjectBase>>(), null);

			//获取当前数据库的缓存器
			var cache = _caches.GetOrAdd(_settings.DatabaseId, new ObjectCache<RedisObjectBase>());
		...
		//Chensoft.Runtime.Caching.ICacheCreator Chensoft.Runtime.Caching.ICache.Creator
		//{
		//	get;
		//	set;
		//}
		...
		{
			return Utility.ConvertValue<T>(GetValue(key));
		}

		//object Chensoft.Runtime.Caching.ICache.GetValue(string key)
		//{
		//	var creator = ((Chensoft.Runtime.Caching.ICache)this).Creator;

		//	if(creator == null)
		//		return this.GetEntry(key);

		//	return GetValue(key, _ =>
		//	{
		//		TimeSpan duration;
		...
		{
			return SetValue(key, value, TimeSpan.Zero, false);
		}
		...
		#region 获取缓存
		//Chensoft.Runtime.Caching.ICache Chensoft.Runtime.Caching.ICacheProvider.GetCache(string name, bool createNotExists = false)
		//{
		//	if(string.IsNullOrWhiteSpace(name))
		//		return this;
		...
		//	return this.GetDictionary(name.Trim()) as Chensoft.Runtime.Caching.ICache;
		//}
		#endregion

		#region 获取队列
		//Chensoft.Collections.IQueue GetQueue(string name)
		//{
